...
cat <<'PROMPT_EOF' > /tmp/coherence_prompt.md
...
Additionally, you MUST check cleanup_system.py and compare its WHITELIST against all active files in the /root directory. Ensure that NO critical files (like logs, databases, JSON states, or new scripts) are missing from the whitelist, to prevent the cleaner from accidentally wiping them out. If any active files are missing, YOU MUST edit cleanup_system.py to add them to the whitelist.
...
for f in *.py GEMINI.md; do
    if [ -f "/root/$f" ]; then
        echo -e "\n=== $f ===" >> /tmp/coherence_prompt.md
        cat "/root/$f" >> /tmp/coherence_prompt.md
    fi
done
...
if agy --model "Gemini 3.1 Pro (High)" --dangerously-skip-permissions --print-timeout 20m0s --print "Please read the instructions and system state from /tmp/coherence_prompt.md and perform the requested audit." > /root/coherence_report.md; then
